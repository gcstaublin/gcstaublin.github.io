...
Regardless of your leaning, all these posts are worth reading and thinking through.

#### My concerns about CSS-in-JS

In his <a href="https://micahgodbolt.com/blog/what-s-right-with-css-in-js/">What's right with CSS-in-JS</a> post, Micah Goldbolt has a couple observations that stuck with me:
...
In my experience, this situation is real, and a problem. Some organizations lack developers who deeply care about writing semantic HTML or well-structured CSS. It's simply not a sexy skill for the developer or the organization. And not one they see the value of. And unless an organization gets hit with a costly ADA lawsuit, they probably won't ever care or even be aware of the subpar experience they're providing to a portion of their audience. And thus, won't do what Micah suggests to keep "front-end minded developers involved in your markup and styles".
...
Beyond, the specifics of writing style in Javascript is the prevailing attitude many developers seem to have about CSS.
...
Many CSS-in-JS approaches inject `<style>` tags into the `<head>` of the document. Doing this obliterates the concept of critical path css, only providing the css for the portion of the interface that is within the viewport for the page load, loading the rest in an async manner. And thus, we have to ask if we're less concerned about performance than we were just a couple years ago.


#### Final thoughts

I see the value in an approach that encapsulates style and keeps developers from wrestling with the cascade or the global nature of CSS.
...
Whether an organization adopts an approach like CSS-in-JS or not, it should be a group decision. It should be a decision that satisfies the needs of the entire organization, not satisfying the desires of a few individual developers.
